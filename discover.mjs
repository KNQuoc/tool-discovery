...
async function discover(url, options = {}) {
  const { crawl = false, maxPages = MAX_CRAWL_PAGES } = options;
...
Examples:
  ...
  node discover.mjs https://resend.com/docs/api-reference --crawl --output resend-tools.json
`);
    process.exit(0);
  }
  ...
  try {
    const result = await discover(url, { crawl, maxPages });
